...
const FontStyles = () => (
  <style>{`
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @keyframes taskDone{0%{opacity:1;transform:translateX(0);background-color:transparent}25%{background-color:#DCFCE7}70%{opacity:.6;transform:translateX(40px);max-height:80px}100%{opacity:0;transform:translateX(80px);max-height:0;padding-top:0;padding-bottom:0;margin:0;overflow:hidden}}
    .task-completing{animation:taskDone .65s cubic-bezier(.4,0,.2,1) forwards;pointer-events:none}
  `}</style>
);
...
const PCOLORS = { 1: "#EF4444", 2: "#F59E0B", 3: "#94A3B8" };
const PSYMBOLS = { 1: "▲▲▲", 2: "▲▲", 3: "▲" };
...
function TasksView({ state, setState }) {
  ...
  const [ctx, setCtx] = useState("all"); const [title, setTitle] = useState(""); const [context, setContext] = useState(categories[0]); const [priority, setPriority] = useState(2); const [dueDate, setDueDate] = useState(""); const [showOpt, setShowOpt] = useState(false); const [newCat, setNewCat] = useState(""); const [showNewCat, setShowNewCat] = useState(false); const [completing, setCompleting] = useState(new Set());
  ...
  return (
    <div className="space-y-5 fade-in">
      ...
      <div className="rounded-xl px-4 py-3 flex flex-wrap gap-x-5 gap-y-1.5" style={{ backgroundColor: C.bgSoft }}><p className="w-full text-[10px] font-extrabold uppercase tracking-widest mb-0.5" style={{ color: C.txS }}>Priority Guide</p>{[[1,"▲▲▲","#EF4444","Critical"],[2,"▲▲","#F59E0B","Important"],[3,"▲","#94A3B8","Nice to do"]].map(([p,sym,col,lbl]) => (<div key={p} className="flex items-center gap-1.5"><span className="text-xs font-extrabold leading-none" style={{ color: col }}>{sym}</span><span className="text-[11px] font-semibold" style={{ color: C.txM }}>{lbl}</span></div>))}</div>
      <Surface><div className="flex gap-2"><input value={title} onChange={e => setTitle(e.target.value)} onKeyDown={e => e.key === "Enter" && add()} placeholder="What needs to get done?" className="flex-1 text-sm rounded-xl px-4 py-3 outline-none font-medium" style={{ backgroundColor: C.bgSoft, color: C.tx }} /><Btn onClick={add} small>Add</Btn></div><button onClick={() => setShowOpt(!showOpt)} className="text-xs font-bold mt-3 inline-flex items-center gap-1" style={{ color: C.txS }}>{showOpt ? <X size={12} /> : <Plus size={12} />} {showOpt ? "Hide" : "Options"}</button>{showOpt && <div className="grid grid-cols-3 gap-2 mt-3"><Sel label="Category" value={context} onChange={e => setContext(e.target.value)}>{categories.map(c => <option key={c} value={c}>{c}</option>)}</Sel><Sel label="Priority" value={priority} onChange={e => setPriority(Number(e.target.value))}><option value={1}>▲▲▲ Critical</option><option value={2}>▲▲ Important</option><option value={3}>▲ Nice to do</option></Sel><Inp label="Due" type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} /></div>}</Surface>
      <div className="space-y-2">{active.length === 0 && <p className="text-sm text-center py-10" style={{ color: C.txS }}>Nothing here</p>}{active.map(t => { const du = dueTag(t); const p = t.priority || 2; const isC = completing.has(t.id); return <div key={t.id} className={`flex items-center gap-3 rounded-xl px-4 py-3.5 hover-lift${isC ? " task-completing" : ""}`} style={{ backgroundColor: C.card, borderLeft: `3px solid ${PCOLORS[p]}`, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}><Check on={isC} onClick={() => complete(t.id)} /><div className="flex-1 min-w-0"><div className="flex items-center gap-2"><span className="text-xs font-extrabold leading-none flex-shrink-0" style={{ color: PCOLORS[p] }}>{PSYMBOLS[p]}</span><p className="text-sm font-medium truncate" style={{ color: C.tx }}>{t.title}</p></div><div className="flex items-center gap-2 mt-1.5"><Chip label={t.context} cat={t.context} />{du && <span className="text-[11px] font-bold px-2 py-0.5 rounded-md" style={{ color: du.color, backgroundColor: du.bg }}>{du.text}</span>}</div></div><button onClick={() => del(t.id)}><X size={16} style={{ color: C.txS }} /></button></div>; })}</div>
...
function TripsView({ state, setState }) {
  const { trips, checklist } = state; const [sel, setSel] = useState(null); const [showAdd, setShowAdd] = useState(false); const [form, setForm] = useState({ destination: "", start_date: "", end_date: "", notes: "" }); const [tab, setTab] = useState("planning"); const [newItem, setNewItem] = useState(""); const [swipedId, setSwipedId] = useState(null); const touchStartX = useRef(0); const didSwipeRef = useRef(false);
  ...
  async function saveTrip() {
    ...
    const t = await db.insertTrip(form);
    if (!t) return;
    const c1 = await db.insertChecklistItem({ trip_id: t.id, title: "Flights booked", list: "planning" });
    const c2 = await db.insertChecklistItem({ trip_id: t.id, title: "Accommodations booked", list: "planning" });
    const packItems = await Promise.all(DEFAULT_PACKING.map(item => db.insertChecklistItem({ trip_id: t.id, title: item, list: "packing" })));
    setState(s => ({ ...s, trips: [...s.trips, t], checklist: [...s.checklist, c1, c2, ...packItems].filter(Boolean) }));
    setShowAdd(false); setForm({ destination: "", start_date: "", end_date: "", notes: "" });
  }
  async function delTrip(id) {
    const items = checklist.filter(c => c.trip_id === id);
    await Promise.all(items.map(c => db.deleteChecklistItem(c.id)));
  ...
  function onTouchEnd(e, id) {
    ...
    if (dx > 50) { setSwipedId(id); didSwipeRef.current = true; }
    else if (dx < -20) { setSwipedId(null); didSwipeRef.current = true; }
    else { didSwipeRef.current = false; }
  }
  ...
  return (
    <div className="space-y-5 fade-in">
      ...
      <div className="space-y-3">
        {trips.length === 0 && <p className="text-sm text-center py-10" style={{ color: C.txS }}>No trips yet</p>}
        {trips.map(t => {
          const its = checklist.filter(c => c.trip_id === t.id && c.list === "planning"); const dn = its.filter(i => i.completed).length; const d = daysUntil(t.start_date); const isSwiped = swipedId === t.id;
          return (
            <div key={t.id} className="relative overflow-hidden rounded-2xl">
              <div className="absolute inset-y-0 right-0 w-20 flex flex-col items-center justify-center rounded-2xl" style={{ backgroundColor: "#EF4444" }}>
                <button onClick={() => delTrip(t.id)} className="flex flex-col items-center gap-1 w-full h-full justify-center">
                  <X size={20} color="white" /><span className="text-white text-[10px] font-bold">Delete</span>
                </button>
              </div>
              <div style={{ transform: `translateX(${isSwiped ? -80 : 0}px)`, transition: "transform 0.25s ease" }}
                onTouchStart={e => onTouchStart(e, t.id)} onTouchEnd={e => onTouchEnd(e, t.id)}
                onClick={() => { if (didSwipeRef.current) { didSwipeRef.current = false; return; } if (isSwiped) { setSwipedId(null); } else { setSel(t.id); } }}>
                <Surface><div className="flex items-start justify-between"><div><p className="text-lg font-bold" style={{ color: C.tx, fontFamily: C.fontH }}>{tripEmoji(t.destination)} {t.destination}</p><p className="text-xs mt-1" style={{ color: C.txS }}>{fmt(t.start_date)} – {fmt(t.end_date)}</p></div><Stat value={d} label="days" size="md" /></div>{its.length > 0 && <div className="mt-3"><Bar value={dn} max={its.length} h={4} /><p className="text-xs mt-1.5" style={{ color: C.txS }}>{dn}/{its.length} planned</p></div>}</Surface>
              </div>
            </div>
          );
        })}
